...
## Работоспособность ≠ ДОСТИЖИМОСТЬ

Второй, более коварный вид пустого теста: он подаёт на вход ровно то, чего
боевой путь не подаёт. Тогда он честно доказывает, что механизм РАБОТАЕТ, и
структурно неспособен заметить, что до механизма никто не доходит.

Живой пример. Определение налога включается только контекстом: при
`context == null` движок молча пропускает правила и сопоставления и берёт код по
умолчанию. `TaxProfileMappingTest` был зелёным годами — потому что звал сервис
напрямую и контекст подавал САМ:

```csharp
Svc.CreateCalculationAsync(le, "OUTPUT", 1000m, …, Ctx(("buyer.id", party)))
```

А из пяти документных путей контекст собирал один. Кредит-нота сторнировала счёт
по другому коду, чем он был начислен, и леджер уходил в минус на разницу ставок.

**Отсюда правило: на каждый механизм, который включается ВХОДНЫМ параметром,
нужен тест, проводящий НАСТОЯЩИЙ документ** — он единственный проверяет, что
параметр кто-то передаёт. И при аудите «работает ли фича» грепай МЕСТА ВЫЗОВА на
наличие необязательного аргумента, а не реализацию. Греп бери с запасом строк:
четырёх строк контекста хватило, чтобы я ошибочно решил, будто контекст не
передаёт вообще никто.
...
## Предусловие, которое тест не проверил, — это его будущий ложный вердикт

Тест, опирающийся на предусловие («поле не задано», «настройка выключена»,
«ячейка без назначения»), обязан это предусловие ПРОВЕРИТЬ, а не предположить.
Иначе его падение сообщает только «что-то не так» и посылает искать ошибку в
боевом коде — которого она не касается.

Живой пример, 2026-09-22. Тест «непройденная приёмка оставляет заказ
черновиком» строился на том, что у типа ячейки назначение не задано
(`Unspecified`), а значит под приёмку она не годится. Тест упал с «факт
Received», и это читалось как дыра в только что написанной команде.

Дыры не было. Включение `InventorySettings.EnforceWarehouseTasks` — **не
пассивный флаг**: его `OnAfterSave` зовёт `PrepareAllYardsAsync()` →
`InferTypePurposesAsync()`, который проставляет назначение типа **по его ИМЕНИ**
(legacy-совместимость). Тип в подготовке назывался `"Receiving"` — и стал
приёмочным ровно в тот момент, когда тест включал дисциплину. Предусловие теста
отменял его же предыдущий шаг.

Три вывода:

1. **Явные assert'ы на предусловие** до основного действия. Одна строка
   превращает «факт Received» в «предусловие: у ячейки назначение не ПРИЁМКА,
   факт Receiving» — и виновник назван:

   ```csharp
   var cells = GetService<IStoreCellService>();
   Assert.IsTrue(await cells.IsWarehouseDisciplineOnAsync(), "предусловие: дисциплина включена");
   var purpose = await cells.GetCellPurposeAsync(cellId);
   Assert.IsTrue(purpose != StoreCellPurpose.Receiving,
       "предусловие: у ячейки назначение не ПРИЁМКА, факт {0}", purpose);
   ```

2. **«Поле не заполнено» — не предусловие, а гипотеза.** Значение, которое
   кто-то может вывести (из имени, из умолчания, хуком), задавай ЯВНО.

3. **Порядок шагов подготовки — часть сценария.** Включение настройки с
   `OnAfterSave` может затереть выставленное до него; ставь явное значение
   ПОСЛЕ такого включения. Прежде чем чинить код, грепай `OnAfterSaveAsync`
   у справочника-настройки.
