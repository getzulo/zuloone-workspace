...
/// primary Contact onto new SalesOrder and SalesRealization.
/// </summary>
public class SalesMasterDataTest : IntegrationTestScriptBase
{
    ...
    // ──────────────────────────────────────────────────────────────────────
    // Test 3: New SalesRealization copies PaymentTerm and Contact from Customer
    // ──────────────────────────────────────────────────────────────────────

    [IntegrationTest("Новый SalesRealization копирует PaymentTerm и Contact из клиента")]
    public async Task SalesInvoiceCopiesPaymentTermAndContact()
    {
        ...
        // Create SalesRealization — OnBeforeSave(isNew=true) should copy PaymentTerm + Contact
        ...
        var loaded = await DocumentManager.GetDocumentAsync<SalesRealization>(invoice.MetaId);
        Assert.IsTrue(loaded is not null, "SalesRealization загружен");
        Assert.IsTrue(loaded!.PaymentTerm == pt.MetaId,
            "SalesRealization.PaymentTerm скопирован из клиента, факт {0}", loaded.PaymentTerm);
        Assert.IsTrue(loaded.Contact == cc.MetaId,
            "SalesRealization.Contact скопирован из основного контакта клиента, факт {0}", loaded.Contact);
    }
}
