...
public class SalesDebitNoteTest : IntegrationTestScriptBase
{
    ...
    private async Task<Setup> SetupAsync(bool configureTax)
    {
        ...
        legalEntity.RegistrationNumber = $"REG-DN-{Db.NewId():N}"[..16];
    ...
    private static async Task<SalesDebitNote> NewNoteAsync(Setup s, Guid invoiceId)
    {
    ...
    private async Task PostNoteAsync(SalesDebitNote note)
    {
        ...
        Assert.IsTrue(run.Success, "команда PostSalesDebitNote: {0}",
    ...
    [IntegrationTest("Дебет-нота начисляет НДС, счёт остаётся Issued")]
    public async Task DebitAddsVatKeepsInvoiceIssued()
    {
        ...
        var storedDraft = await DocumentManager.GetDocumentAsync<SalesDebitNote>(note.MetaId);
        Assert.IsTrue(storedDraft!.Lines.Count == 0, "строки со счёта не копируются, факт {0}", storedDraft.Lines.Count);
        Assert.IsTrue(storedDraft.TaxRateApplied == inv.TaxRateApplied,
            "ставка скопирована со счёта, факт {0}", storedDraft.TaxRateApplied);

        storedDraft.Lines.Add(new SalesDebitNoteLinesTablePartRow
        {
            Item = s.Item,
            Quantity = 2m,
            UnitPrice = 10m,
        });
        await DocumentManager.SaveDocumentAsync(storedDraft);
        await PostNoteAsync(storedDraft);
        ...
        Assert.IsTrue(await ReceivableAsync(s) == 118m,
            "долг 115 + НДС 3 = 118, факт {0}", await ReceivableAsync(s));
        Assert.IsTrue(await VatPayableAsync(s) == 18m,
            "VatPayable 15 + 3 = 18, факт {0}", await VatPayableAsync(s));

        var calc = await LinkedCalcAsync(note.MetaId);
        Assert.IsNotNull(calc, "после проведения есть TaxCalculation");
        Assert.IsTrue(calc!.Lines[0].TaxBase == 20m, "база +20, факт {0}", calc.Lines[0].TaxBase);
        Assert.IsTrue(calc.Lines[0].TaxAmount == 3m, "налог +3, факт {0}", calc.Lines[0].TaxAmount);
    }

    [IntegrationTest("Пустую дебет-ноту проводить нельзя")]
    public async Task EmptyLinesRefusePost()
    {
        var s = await SetupAsync(configureTax: true);
        var inv = await IssueAsync(s, 4m, 25m);
        ...
        try
        {
            note.Subtype = SalesDebitNote.Subtypes.Posted;
            await DocumentManager.SaveDocumentAsync(note);
        }
        catch (Exception ex)
        {
        ...
        Assert.IsTrue(reason.Contains("строки"), "отказ про пустые строки, факт: {0}", reason);
        Assert.IsTrue(await ReceivableAsync(s) == 115m, "долг не двигается, факт {0}", await ReceivableAsync(s));
    }
    ...
    public async Task ExtraLineBeyondInvoiceAllowed()
    {
        ...
        Assert.IsTrue(await ReceivableAsync(s) == 133.75m,
            "115 + 18.75 = 133.75, факт {0}", await ReceivableAsync(s));
        var calc = await LinkedCalcAsync(note.MetaId);
        Assert.IsTrue(calc!.Lines[0].TaxBase == 125m, "база сверх счёта +125, факт {0}", calc.Lines[0].TaxBase);
    }

    [IntegrationTest("Дебет-нота только по реализованному счёту")]
    public async Task RefusesDraftInvoice()
    {
        ...
        note.Lines.Add(new SalesDebitNoteLinesTablePartRow { Item = s.Item, Quantity = 1m, UnitPrice = 10m });
    ...
    [IntegrationTest("Без НДС на счёте дебет-ноту проводить нечего")]
    public async Task RefusesWhenInvoiceHasNoTax()
    {
        ...
        var note = await NewNoteAsync(s, inv.MetaId);
        note.Lines.Add(new SalesDebitNoteLinesTablePartRow { Item = s.Item, Quantity = 2m, UnitPrice = 10m });
        await DocumentManager.SaveDocumentAsync(note);
