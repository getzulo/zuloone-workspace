...
// Live line calculator: picking Item fills Unit (item card) and UnitPrice
// (customer / contract price list). Owner<SalesOrder> is the in-memory header
// — works before the document is saved.
public partial class SalesOrderLinesEventHandler : TypedTablePartEventHandler<SalesOrderLinesTablePartRow>
{
    ...
    private static async Task FillAsync(
        SalesOrderLinesTablePartRow row, SalesOrder? header, string fieldName, EventContext context)
    {
