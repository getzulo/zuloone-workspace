...
public partial class CustomerPaymentXrPrintForm : PrintFormBase
{
    ...
    public override async Task<SlimTable> GetDataAsync(PrintFormContext context)
    {
        ...
        var doc = await documents.GetDocumentAsync<CustomerPayment>(context.RecordId);
        if (doc == null)
        {
        ...
        // The money is already per line; the total is their sum, no pricing service.
        decimal subTotal = 0m;
        ...
        {
            n++;
            var linePartyText = await NameAsync(display, "Customer", line.Customer, ct);
            var lineNoteText = await NameAsync(display, "SalesContract", line.Contract, ct);
            table.Add(Row(
                doc.ID ?? "",
