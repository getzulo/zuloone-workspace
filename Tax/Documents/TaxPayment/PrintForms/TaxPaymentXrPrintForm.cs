...
// Tax payment paper: legal entity, tax code, line amount. Mirror of VendorPaymentXr.
public partial class TaxPaymentXrPrintForm : PrintFormBase
{
    ...
    public override async Task<SlimTable> GetDataAsync(PrintFormContext context)
    {
        ...
        var doc = await documents.GetDocumentAsync<TaxPayment>(context.RecordId);
        if (doc == null)
        {
        ...
        {
            n++;
            var code = await NameAsync(display, "TaxCode", line.TaxCode, ct);
            table.Add(Row(
                doc.ID ?? "",
