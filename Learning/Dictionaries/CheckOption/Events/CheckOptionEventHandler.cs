...
// Handler for CheckOption. Override only the hooks you need.
// This is the OWNER: it is link 0 of the chain, so it does not call next().
// A handler in ANOTHER model declares itself with [ExtensionOf("CheckOption")] and
...
public partial class CheckOptionEventHandler : TypedDictionaryEventHandler<CheckOption>
{
    ...
    // public override async Task<EventResult> OnFieldChangedAsync(CheckOption record, string fieldName, object? value, EventContext context)
    // {
    //     return EventResult.Ok();
    // }
}
