...
public partial class PurchasingSettingsEventHandler : TypedDictionaryEventHandler<PurchasingSettings>
{
    ...
    public override async Task<EventResult> OnBeforeCreateAsync(PurchasingSettings record, EventContext context){
    ...
    public override async Task<EventResult> OnBeforeSaveAsync(PurchasingSettings record, bool isNew, EventContext context){
    ...
    public override async Task<EventResult> OnAfterLoadAsync(PurchasingSettings record, EventContext context)
    {
        var prior = await next(record, context);
        if (!prior.Success) return prior;
