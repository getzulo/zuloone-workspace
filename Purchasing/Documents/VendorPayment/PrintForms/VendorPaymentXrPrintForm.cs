...
public partial class VendorPaymentXrPrintForm : PrintFormBase
{
    ...
    public override async Task<SlimTable> GetDataAsync(PrintFormContext context)
    {
        ...
        var doc = await documents.GetDocumentAsync<VendorPayment>(context.RecordId);
        if (doc == null)
        {
        ...
        {
            n++;
            var linePartyText = await NameAsync(display, "Supplier", line.Supplier, ct);
            table.Add(Row(
                doc.ID ?? "",
