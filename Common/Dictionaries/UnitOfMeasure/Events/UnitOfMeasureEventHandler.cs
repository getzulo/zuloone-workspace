...
// Strongly-typed lifecycle handler for UnitOfMeasure records (MIQS DictionaryEventHandlerBase<T>).
// `record` is a typed UnitOfMeasure entity — access fields directly (record.SomeField).
...
public partial class UnitOfMeasureEventHandler : TypedDictionaryEventHandler<UnitOfMeasure>
{
    // Building a new record server-side: seed default field values here.
    public override async Task<EventResult> OnBeforeCreateAsync(UnitOfMeasure record, EventContext context){
    ...
    public override async Task<EventResult> OnBeforeSaveAsync(UnitOfMeasure record, bool isNew, EventContext context){
    ...
    public override Task<EventResult> OnAfterSaveAsync(UnitOfMeasure record, bool isNew, EventContext context)
    ...
    //public override Task<EventResult> OnBeforeInsertAsync(UnitOfMeasure record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterInsertAsync(UnitOfMeasure record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnBeforeUpdateAsync(UnitOfMeasure record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterUpdateAsync(UnitOfMeasure record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    ...
    public override Task<EventResult> OnBeforeCloneAsync(UnitOfMeasure record, EventContext context)
        => next(record, context);

    // After a record is loaded: compute transient/derived property values.
    public override Task<EventResult> OnAfterLoadAsync(UnitOfMeasure record, EventContext context)
        => next(record, context);

    // Validate a single field (name + current value).
    public override Task<EventResult> OnValidateFieldAsync(UnitOfMeasure record, string fieldName, object? value, EventContext context)
        => next(record, fieldName, value, context);

    // An insert/update failed: return Error("friendly text") to replace the raw DB error.
    public override Task<EventResult> OnSaveFailedAsync(UnitOfMeasure record, string errorMessage, EventContext context)
