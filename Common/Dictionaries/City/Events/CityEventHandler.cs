...
// Strongly-typed lifecycle handler for City records (MIQS DictionaryEventHandlerBase<T>).
// `record` is a typed City entity — access fields directly (record.SomeField).
...
public partial class CityEventHandler : TypedDictionaryEventHandler<City>
{
    // Building a new record server-side: seed default field values here.
    public override async Task<EventResult> OnBeforeCreateAsync(City record, EventContext context){
    ...
    public override async Task<EventResult> OnBeforeSaveAsync(City record, bool isNew, EventContext context){
    ...
    public override Task<EventResult> OnAfterSaveAsync(City record, bool isNew, EventContext context)
    ...
    //public override Task<EventResult> OnBeforeInsertAsync(City record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterInsertAsync(City record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnBeforeUpdateAsync(City record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterUpdateAsync(City record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    ...
    public override Task<EventResult> OnBeforeCloneAsync(City record, EventContext context)
        => next(record, context);

    // After a record is loaded: compute transient/derived property values.
    public override Task<EventResult> OnAfterLoadAsync(City record, EventContext context)
        => next(record, context);

    // Validate a single field (name + current value).
    public override Task<EventResult> OnValidateFieldAsync(City record, string fieldName, object? value, EventContext context)
        => next(record, fieldName, value, context);

    // An insert/update failed: return Error("friendly text") to replace the raw DB error.
    public override Task<EventResult> OnSaveFailedAsync(City record, string errorMessage, EventContext context)
