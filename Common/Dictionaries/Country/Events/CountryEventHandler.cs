...
// Strongly-typed lifecycle handler for Country records (MIQS DictionaryEventHandlerBase<T>).
// `record` is a typed Country entity — access fields directly (record.SomeField).
...
public partial class CountryEventHandler : TypedDictionaryEventHandler<Country>
{
    // Building a new record server-side: seed default field values here.
    public override async Task<EventResult> OnBeforeCreateAsync(Country record, EventContext context){
        ...
        var createSeed = await createDefaults.SeedAsync("Country");
    ...
    public override Task<EventResult> OnAfterSaveAsync(Country record, bool isNew, EventContext context)
    ...
    //public override Task<EventResult> OnBeforeInsertAsync(Country record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterInsertAsync(Country record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnBeforeUpdateAsync(Country record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterUpdateAsync(Country record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    ...
    public override Task<EventResult> OnBeforeCloneAsync(Country record, EventContext context)
        => next(record, context);

    // After a record is loaded: compute transient/derived property values.
    public override Task<EventResult> OnAfterLoadAsync(Country record, EventContext context)
        => next(record, context);

    // Validate a single field (name + current value).
    public override Task<EventResult> OnValidateFieldAsync(Country record, string fieldName, object? value, EventContext context)
        => next(record, fieldName, value, context);

    // An insert/update failed: return Error("friendly text") to replace the raw DB error.
    public override Task<EventResult> OnSaveFailedAsync(Country record, string errorMessage, EventContext context)
