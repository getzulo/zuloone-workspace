...
// Same Annex 2 printout as ZatcaInvoiceXr, for a posted debit note.
// Standard stays gated by BuyerReleaseBlockAsync until TaxDocument is Cleared.
public partial class ZatcaDebitNoteXrPrintForm : PrintFormBase
{
    ...
    public override async Task<SlimTable> GetDataAsync(PrintFormContext context)
    {
        ...
        var note = await documents.GetDocumentAsync<SalesDebitNote>(context.RecordId);
        if (note == null)
        {
            table.Add(Empty());
            return table;
        ...
        var row = await data.GetByIdAsync("SalesDebitNote", note.MetaId);
        ...
        var title = string.Equals(zatcaType, "Standard", StringComparison.OrdinalIgnoreCase)
            ? "Tax Debit Note / إشعار مدين ضريبي"
            : "Simplified Tax Debit Note / إشعار مدين ضريبي مبسط";
