...
public partial class UaVatFirstEventTotalDriver
{
        ...
        Guid docId)
    {
        ...
        if (excess > 0m)
        {
            // ПРЕВЫШЕНИЕ — ЭТО НЕ ТОЛЬКО СТАВКА. С первого числа месяца,
            // следующего за кварталом превышения, спрощенець переходит на общую
            // систему (ПКУ 293.8). Решается это сейчас, а случается через недели,
            // поэтому здесь только ЗАПИСЬ в журнал; переставит режим задание,
            // когда дата придёт. Запись идемпотентна: драйвер может отработать по
            // документу не один раз.
            await firstEvent.ScheduleForcedChangeAsync(key.Entity, regime, movementDate);
