...
// Бланк J0510111 (додаток Д1). Категорія ЗО 1, тип нарахувань 1, G14 — дні відносин.
public partial class UaDpsD1XrPrintForm : PrintFormBase
{
    ...
    public override async Task<SlimTable> GetDataAsync(PrintFormContext context)
    {
        ...
        var rows = await context.GetService<IUaTaxFiling>()
            .ListDpsD1Async(doc.LegalEntity, doc.PeriodFrom, doc.PeriodTo);
        var n = 0;
        var total = 0m;
        foreach (var row in rows)
        {
            n++;
            total += row.EmployerEsv;
            table.Add(Row(
                doc.ID ?? "",
        ...
        if (n == 0)
            table.Add(Row(doc.ID ?? "", DateText(doc.DocumentDate), seller, period, "J0510111", 0, "", "", "", 0m, 0m, 0m));
        return table;
    }
