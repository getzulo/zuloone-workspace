...
public class UkraineSeededRatesTest : IntegrationTestScriptBase
{
    ...
    public async Task SingleTaxOwnsItsTax()
    {
        ...
        // безкатегорийную полосу налога и спорят между собой. Это и есть то,
        // что здесь стерегут — не количество ставок само по себе.
        //
        // Ставок ПЯТЬ. Обычные 3% и 5%; 15% на превышение годового предела
        // (ПКУ 293.4); и ПОДВІЙНА ставка 6% / 10% (ПКУ 293.5) — 6% это двойная
        // к 3%, 10% двойная к 5%. Подвійна применяется не к превышению, а к
        // негрошовим розрахункам и забороненому виду діяльності: на них в
        // поставке висят коды UA-EPB3/UA-EPB5 и UA-EPF3/UA-EPF5, и все четыре
        // читаются кодом — это живой контур, а не каталог про запас.
        //
        // Перечисление ТОЧНОЕ намеренно: новая ставка в поставке обязана
        // приехать вместе с решением, в какой она категории. Список, который
        // молча растёт, — это как раз спорящие полосы, от которых тест и
        // поставлен. Добавил ставку — впиши её сюда и объясни здесь, чем она.
        var rates = await Rates.GetRecordsAsync($"Tax = '{tax.MetaId}'");
        var codes = rates.Select(r => r.Code).OrderBy(c => c).ToList();
        Assert.IsTrue(codes.SequenceEqual(new[] { "EP10", "EP15", "EP3", "EP5", "EP6" }),
            "у EP-UA ровно ставки EP3, EP5, EP6, EP10 и EP15, факт [{0}]", string.Join(", ", codes));
