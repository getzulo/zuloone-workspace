...
## 4б. Пишешь движения руками — снимай их тоже руками, и НЕ по документу

Движения транзакционного скрипта платформа сторнирует при аннулировании сама.
Записанные из обработчика (`PostMovementAsync`) — НЕ сторнирует: снятие пишешь
ты, в `OnAfterPost` при переходе в отменяющий подтип.

**И вот где легко снести чужое.** `DeleteDocumentMovementsAsync(register, doc)`
удаляет движения документа **по регистру целиком, а не по автору**. Если в тот
же регистр тем же документом пишет tx-скрипт модели-владельца — а так и будет,
раз ты расширяешь её документ, — твой «откат» унесёт и её проводку.

Правило: **удалять можно только там, где ты единственный писатель** (свой
регистр; чужой, в который по документам этого типа больше никто не ходит).
Там, где писателей двое, откат — ВСТРЕЧНОЕ движение, а не удаление.

Сколько возвращать — **читай из своего регистра, а не пересчитывай**: ставка,
курс или настройка с тех пор могли смениться, и пересчёт вернёт не ту сумму,
которую списал.

И идемпотентность: движение из `OnAfterPost` поднимает пересчёт итогов, а тот
снова проходит цепочку событий — проведение отработает по документу не один
раз. Ключ — сам документ: есть твои движения в своём регистре, второй проход не
нужен. Тот же ключ снимается при откате, поэтому «снято» и «можно начислить
заново» становятся одним состоянием.

**А если чужая модель считает себя единственной, кто трогает её регистр** —
это не обходится расширением, это чинится у владельца. У HR аннулирование ФОТ
объясняло недостачу остатка ровно одним своим удержанием; появление второго
сделало бы каждое начисление неаннулируемым. Лечится не веткой «если Украина»,
а ресурсом `PayrollLiability.Withheld`, которым ЛЮБОЙ удерживающий объявляет
себя, — владелец про твою страну по-прежнему не знает.
