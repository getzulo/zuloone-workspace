...
Аудит всего воркспейса — CI: `.github/scripts/check_model_deps.py` после
JSON-ссылок сканирует `.cs` вне `Tests/` теми же формами вызова, что
платформенный `ScriptNameReferences`, и сверяет имя с `kind: "Register"` /
`"GlobalConstant"` против замыкания зависимостей. Прогонять локально:
