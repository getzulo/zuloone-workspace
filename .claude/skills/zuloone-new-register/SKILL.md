...
**Две причины всё-таки завести `dimensions`, и обе про ЧТЕНИЕ.** Первая
известная: драйвер итогов видит только координаты, аналитик `TransactionBase`
не знает вовсе. Вторая всплывает позже и больнее: в таблице движений `TR_`
аналитики лежат ОДНИМ идентификатором набора, а не колонками. Поэтому
`QueryMovementsAsync(registerId, "Employee = '…'")` по аналитике не отфильтрует
и разложить движения по работнику не даст — а по измерению вернёт обычную
колонку. Балансовому чтению это не мешает: `GetBalanceAsync` аналитики
переагрегирует из `TR_` сам. Правило простое: разрез, по которому ты
собираешься ЧИТАТЬ СЫРЫЕ ДВИЖЕНИЯ (персонифицированная отчётность, сторно
своих записей, сверка), делай измерением.
