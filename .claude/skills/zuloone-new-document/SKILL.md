...
ещё раз. Всё, что оно порождает, — связанный документ, проводка в GL,
начисленные баллы — порождается ДВАЖДЫ.
...
зелёным, пока в проме каждая продажа товара со слоями удваивает выручку в
книге. Ровно так этот баг и прожил незамеченным.
...
Ключ идемпотентности бери из источника (id документа + назначение), а не из
времени или счётчика. Проверка «уже сделано» обязана переживать и отмену с
