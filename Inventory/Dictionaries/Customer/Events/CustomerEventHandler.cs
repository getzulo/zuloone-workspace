...
// Strongly-typed lifecycle handler for Customer records (MIQS DictionaryEventHandlerBase<T>).
// `record` is a typed Customer entity — access fields directly (record.SomeField).
...
public partial class CustomerEventHandler : TypedDictionaryEventHandler<Customer>
{
    // Building a new record server-side: seed default field values here.
    public override async Task<EventResult> OnBeforeCreateAsync(Customer record, EventContext context){
    ...
    public override async Task<EventResult> OnBeforeSaveAsync(Customer record, bool isNew, EventContext context){
    ...
    public override Task<EventResult> OnAfterSaveAsync(Customer record, bool isNew, EventContext context)
    ...
    //public override Task<EventResult> OnBeforeInsertAsync(Customer record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterInsertAsync(Customer record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnBeforeUpdateAsync(Customer record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterUpdateAsync(Customer record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    ...
    public override Task<EventResult> OnBeforeCloneAsync(Customer record, EventContext context)
        => next(record, context);

    // After a record is loaded: compute transient/derived property values.
    public override Task<EventResult> OnAfterLoadAsync(Customer record, EventContext context)
        => next(record, context);

    // Validate a single field (name + current value).
    public override Task<EventResult> OnValidateFieldAsync(Customer record, string fieldName, object? value, EventContext context)
        => next(record, fieldName, value, context);

    // An insert/update failed: return Error("friendly text") to replace the raw DB error.
    public override Task<EventResult> OnSaveFailedAsync(Customer record, string errorMessage, EventContext context)
