...
// Strongly-typed lifecycle handler for Warehouse records (MIQS DictionaryEventHandlerBase<T>).
// `record` is a typed Warehouse entity — access fields directly (record.SomeField).
...
public partial class WarehouseEventHandler : TypedDictionaryEventHandler<Warehouse>
{
    // Building a new record server-side: seed default field values here.
    public override Task<EventResult> OnBeforeCreateAsync(Warehouse record, EventContext context)
    {
    ...
    public override Task<EventResult> OnBeforeSaveAsync(Warehouse record, bool isNew, EventContext context)
    {
    ...
    public override Task<EventResult> OnAfterSaveAsync(Warehouse record, bool isNew, EventContext context)
    ...
    //public override Task<EventResult> OnBeforeInsertAsync(Warehouse record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterInsertAsync(Warehouse record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnBeforeUpdateAsync(Warehouse record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterUpdateAsync(Warehouse record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    ...
    public override Task<EventResult> OnBeforeCloneAsync(Warehouse record, EventContext context)
        => Task.FromResult(EventResult.Ok());

    // After a record is loaded: compute transient/derived property values.
    public override Task<EventResult> OnAfterLoadAsync(Warehouse record, EventContext context)
        => Task.FromResult(EventResult.Ok());

    // Validate a single field (name + current value).
    public override Task<EventResult> OnValidateFieldAsync(Warehouse record, string fieldName, object? value, EventContext context)
        => Task.FromResult(EventResult.Ok());

    // An insert/update failed: return Error("friendly text") to replace the raw DB error.
    public override Task<EventResult> OnSaveFailedAsync(Warehouse record, string errorMessage, EventContext context)
        => Task.FromResult(EventResult.Ok());
