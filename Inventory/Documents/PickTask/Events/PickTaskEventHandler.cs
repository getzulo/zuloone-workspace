    ...
    public override async Task<EventResult> OnBeforePostAsync(PickTask header, EventContext context)
    {
        ...
        foreach (var line in lines)
        {
            if (enforcing && !await cells.IsCellAllowedForAsync(line.ToCell, StoreCellPurpose.Picking))
                return EventResult.Cancel("Отбор кладёт товар в ячейку ОТБОРА — у выбранной ячейки другое назначение");
        ...
        foreach (var kv in demand)
        {
            ...
            if (kv.Value > onHand)
                return EventResult.Cancel($"Недостаточно товара в ячейке хранения: требуется {kv.Value}, в наличии {onHand}");
        }
