...
// Quantity-only adjustment slip: one cell, signed quantity, reason in Notes.
public partial class StockAdjustmentXrPrintForm : PrintFormBase
{
    ...
    public override async Task<SlimTable> GetDataAsync(PrintFormContext context)
    {
        ...
        var doc = await documents.GetDocumentAsync<StockAdjustment>(context.RecordId);
        if (doc == null)
        {
            table.Add(Row("", "", "", "", 0, "", 0m, ""));
            return table;
        ...
        foreach (var line in doc.Lines)
        {
            ...
            table.Add(Row(
                doc.ID ?? "", DateText(doc.DocumentDate), cell, reason,
