...
public partial class TaxCalculationGLEventHandler : TypedDocumentEventHandler<TaxCalculation>
{
    ...
    private static async Task<Guid?> ResolveSourceAsync(IDocumentManager docs, TaxCalculation calc)
    {
        ...
        if (reason.StartsWith(salesPrefix, StringComparison.Ordinal))
        {
            ...
            {
                var byId = await docs.GetDocumentAsync<SalesRealization>(invoiceId);
                if (byId != null) return byId.MetaId;
            }
            var invoices = await docs.QueryDocumentsAsync<SalesRealization>($"ID = '{escaped(number)}'");
            if (invoices.Count == 0)
                invoices = await docs.QueryDocumentsAsync<SalesRealization>($"Number = '{escaped(number)}'");
