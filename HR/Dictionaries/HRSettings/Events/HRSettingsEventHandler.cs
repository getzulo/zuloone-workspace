...
// Strongly-typed lifecycle handler for HRSettings records (MIQS DictionaryEventHandlerBase<T>).
// `record` is a typed HRSettings entity — access fields directly (record.SomeField).
...
public partial class HRSettingsEventHandler : TypedDictionaryEventHandler<HRSettings>
{
    // Building a new record server-side: seed default field values here.
    public override async Task<EventResult> OnBeforeCreateAsync(HRSettings record, EventContext context){
    ...
    public override async Task<EventResult> OnBeforeSaveAsync(HRSettings record, bool isNew, EventContext context){
    ...
    public override Task<EventResult> OnAfterSaveAsync(HRSettings record, bool isNew, EventContext context)
    ...
    //public override Task<EventResult> OnBeforeInsertAsync(HRSettings record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterInsertAsync(HRSettings record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnBeforeUpdateAsync(HRSettings record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    //public override Task<EventResult> OnAfterUpdateAsync(HRSettings record, EventContext context)
    //    => Task.FromResult(EventResult.Ok());
    ...
    public override Task<EventResult> OnBeforeCloneAsync(HRSettings record, EventContext context)
        => next(record, context);

    // After a record is loaded: compute transient/derived property values.
    public override Task<EventResult> OnAfterLoadAsync(HRSettings record, EventContext context)
        => next(record, context);

    // Validate a single field (name + current value).
    public override Task<EventResult> OnValidateFieldAsync(HRSettings record, string fieldName, object? value, EventContext context)
        => next(record, fieldName, value, context);

    // An insert/update failed: return Error("friendly text") to replace the raw DB error.
    public override Task<EventResult> OnSaveFailedAsync(HRSettings record, string errorMessage, EventContext context)
