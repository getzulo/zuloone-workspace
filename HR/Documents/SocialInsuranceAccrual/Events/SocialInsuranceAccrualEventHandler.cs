...
public partial class SocialInsuranceAccrualEventHandler : TypedDocumentEventHandler<SocialInsuranceAccrual>
{
    ...
    public override async Task<EventResult> OnBeforePostAsync(SocialInsuranceAccrual header, EventContext context){
    ...
    public override async Task<EventResult> OnGenerateDescriptionAsync(SocialInsuranceAccrual header, EventContext context){
        var prior = await next(header, context);
        if (!prior.Success) return prior;
