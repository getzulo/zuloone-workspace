...
public partial class PayrollPaymentEventHandler : TypedDocumentEventHandler<PayrollPayment>
{
    ...
    public override async Task<EventResult> OnBeforePostAsync(PayrollPayment header, EventContext context){
    ...
    public override async Task<EventResult> OnGenerateDescriptionAsync(PayrollPayment header, EventContext context){
        var prior = await next(header, context);
        if (!prior.Success) return prior;
